...
export function createTools(options = {}) {
  ...
  // Model-facing schema must describe INPUT: a field carrying `.default()` is optional on the wire
  // (the tool fills it), so the default `io: 'output'` projection would wrongly mark it required.
  // `$schema` is stripped because DSH validates only `output.schema` and the key is pure noise.
  const jsonSchema = schema => { const { $schema, ...rest } = z.toJSONSchema(schema, { io: 'input' }); return rest }
  const make = (name, description, schema, execute, readOnly = true) => ({ name, description, schema,
    parameters:jsonSchema(schema), annotations:{readOnlyHint:readOnly, destructiveHint:false, idempotentHint:readOnly, openWorldHint:true},
