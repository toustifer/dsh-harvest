...
function apply(ctx, config = {}) {
  ...
  const tools = [
    ...
    defineTool({
      name: 'harvest_scout',
      description: '多平台并行发现候选来源：GitHub 代码、Web/Exa 搜索、Twitter、Reddit、小红书、YouTube。任一道失败自动记入 skipped 不阻塞；返回 5-15 条带 URL 的候选源。',
      parameters: {
    ...
    defineTool({
      ...
      description: '逐条抓取 URL 正文：静态页直抓，403/JS 页自动升级 r.jina.ai，paywall(402) 与不可达分别标记，不静默丢弃。',
      parameters: {
        type: 'object', additionalProperties: false,
        properties: {
    ...
    defineTool({
      ...
      async execute(args) {
        const matrix = (args.sources || []).map((s) => {
          ...
          if (['official', 'code', 'report'].includes(t) || /(api|docs|github|arxiv)\./.test(u)) firsthand = 4
          if (t === 'social') { firsthand = 1; authority = 1; bias = 1 }
          if (t === 'unknown') { verifiable = 1; authority = 1 }
          if (t === 'report' || t === 'media') { verifiable = 3; timeliness = 3 }
