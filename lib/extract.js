...
// 自动检测编码并转换 Buffer 为字符串
function decodeBuffer(buffer) {
  // 取前 1024 字节探测 charset 声明
  const head = buffer.toString('ascii', 0, 1024)
  const match = head.match(/charset=["']?([a-zA-Z0-9\-_]+)["']?/i)
  const charset = match ? match[1].toLowerCase() : 'utf-8'

  if (['gbk', 'gb2312', 'gb18030'].includes(charset)) {
    try {
...
async function direct(url) {
  const { status, buffer } = await httpGet(url, { timeoutMs: 20000 })
...
async function viaJina(url) {
  const { status, buffer } = await httpGet(JINA + url, { timeoutMs: 30000 })
