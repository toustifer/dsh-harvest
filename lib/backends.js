...
export const BACKENDS = {
  ...
  v2ex: {
    ...
    parse: (out, platform, query, limit) => {
      ...
      if (toks.length) items = items.filter((x) => toks.some((t) => String(x.title).toLowerCase().includes(t)))
  ...
  linkedin: {
    label: 'LinkedIn',
    kind: 'social',
    build: (query, n) => ({ file: 'mcporter', args: ['call', `linkedin-scraper.search_people(keyword: "${query}", limit: ${n})`] }),
    parse: (out, platform) => normalizeItems(tryJson(out), platform),
  },
}
